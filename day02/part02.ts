 ...
 * @returns
 */
export const levelChecks = (levels: number[]) => {
  ...
  const isConsistent = levels.every((level, index) => {
    if (index === levels.length - 1) return true;
    return isIncreasing ? level < levels[index + 1] : level > levels[index + 1];
  });
 ...
 * @returns
 */
export const levelIsSafe = (level: string): boolean => {
  const levels = parseLevel(level);
  if (levelChecks(levels)) return true;

  for (let i = 0; i < levels.length; i++) {
    const levelsCopy = levels.slice();
    levelsCopy.splice(i, 1);
    if (levelChecks(levelsCopy)) return true;
  }
  return false;
};
...
export const safeReports = (reports: string): boolean[] => {
  ...
  return parsedReports.map(levelIsSafe);
};
